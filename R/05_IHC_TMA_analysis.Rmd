...
Whole slide validation analysis

Read in all of the single-cell data tsv's and calculate average expression/ highly expressing cells
```{r}
#All project tsvs for each patient
files <- list.files("/mnt/Validation_LNmets/singlecelldat/")

#Read in all cells and set thresholds over all
out <- list()
counter <- 1
for (i in files){
  d <- fread(paste0("/mnt/Validation_LNmets/singlecelldat/",i))
  d <- d[,c("Image","Nucleus: DAB OD mean","Cell: DAB OD mean","Nucleus: Area","Cell: Area")]
  out[[counter]]<- d
  counter = counter + 1
}
d <- do.call(rbind, out)
...
#Name corrections
d[Image == "GATA3_2613LN-1.czi",Image := "PR_2613LN.czi"]
d[str_detect(Image,"3921LN-1.czi"),Image := gsub("-1.czi","2.czi",Image)]
d[str_detect(Image," - Scene #0"),Image := gsub(" - Scene #0","",Image)]

#Extract marker and PID and nr LN from image name
d[,marker := gsub("_.*","",Image)]
d[marker == "KI67",marker := "Ki67"]
d[marker == "P53",marker := "p53"]
d[,PID := gsub(".*_","",Image)]
d[,PID := gsub(".czi","",PID)]
d[,nLN := gsub(".*LN","",PID)]
d[nLN == "",nLN := "1"]
...
#More cleaning vs metadta
d[PID == "3242",PID := "3243"]

#Kick all black stains out
d <- d[!(PID == "776" & marker == "HER2"),]
d <- d[PID != "4085",]

#Filter out very small (immune) cells that may have been captured in ROI accidentally
#ggplot(d,aes(x = asinh(`Nucleus: Area`)))+geom_density()+facet_wrap(~marker)
#d <- d[asinh(`Nucleus: Area`) > 3.5,]

#Extract number of cells (should only be tumor cells due to ROI selection)
d[,ncells := .N, by = c("Image")]

#Arcsinh transform cell means
d[,arcsinh := asinh(`Cell: DAB OD mean`/0.5)]

#Average tumor cell expression levels per image(for patient average averages of individual images bellow because when there are multiple LNs per patient not the same number of cells are sampled of each LN)
#Average on raw counts, not arcsinh transformed (as in TMAs above)!
d[,avgImage := mean(`Cell: DAB OD mean`,na.rm = T), by = c("Image")]

#Fraction of highly expression cells (check if raw or arcsinh thresh better)
d$HiLo <- "low"
d[arcsinh > 0.5, HiLo := "high"]
d[,N := .N,by = c("Image","HiLo")]
d[,frac := N/.N,by = c("Image")]

#Single-cell distributions
ggplot(d,aes(x = arcsinh))+geom_density()+facet_wrap(~marker)
```

How different are multiple LNs of the same patient?
```{r}
#Get patients with multiple LNs
mult <- unique(d[,c("PID","nLN")])
mult[,nr := .N,by = "PID"]
mult <- mult[nr > 1,]
mult <- unique(d[PID %in% unique(mult$PID),c("PID","nLN","avgImage","marker","Image")])

#Normalize the image means per marker because otherwise markers that are expressed in all cells get much higher image means than markers like Ki67 -> higher variability across different LNs
normalize01 <- function(x, na.rm = TRUE) {
    return((x- min(x)) /(max(x)-min(x)))
}
mult[,avgImageNorm := normalize01(avgImage),by = marker]
mult[,avgPID := mean(avgImageNorm,na.rm = T), by = c("PID","marker")]
mult <- dcast.data.table(mult, "PID + marker + avgPID ~ nLN",value.var = "avgImageNorm")
setnames(mult,"1","LN1")
setnames(mult,"2","LN2")
setnames(mult,"3","LN3")

#Calculate standard deviation to mean across images of patient
mult[is.na(LN3),stDeviation := sqrt(((avgPID - LN1)^2 + (avgPID - LN2)^2)/2)]
mult[!is.na(LN3),stDeviation := sqrt(((avgPID - LN1)^2 + (avgPID - LN2)^2 + (avgPID - LN3)^2)/3)]
mult[is.na(LN1),stDeviation := sqrt(((avgPID - LN2)^2 + (avgPID - LN3)^2)/2)]

ggplot(mult,aes(x = avgPID,color = marker)) +geom_density()

ggplot(mult,aes(y = stDeviation,x = marker))+geom_boxplot()

ggplot(mult,aes(x = avgPID,y = stDeviation))+geom_point()+facet_wrap(~marker)
...
```{r}
library("readxl")
meta = read_excel("/mnt/Validation_LNmets/metadata_final.xlsx")
meta = as.data.table(meta)

#Clean censoring
meta$censoring = 0
meta[(`Reconsent Ja/Nein` == "verstorben") & (`mBC_JA-NEIN` == "J" | `mBC_JA-NEIN` == "pM1" | `mBC_JA-NEIN` == "OSt < 50 m"),censoring := 1]
#meta[`Reconsent Ja/Nein` == "verstorben",censoring := 1] #more dead -> even clearer to illustrate non-prop hazard -> some aspects even more significant

#Clean OS
setnames(meta,"OS time in month", "OS")
meta[OS == "na", OS := NA]
meta$OS = as.numeric(meta$OS)

#Clean molecular subtype
meta[,pN := substr(N,1,4)]
meta[(ER_NEGPOS == "P" | PR_NEGPOS == "P")&HER2_NEGPOS == "P",mol := "HR+HER2+"]
meta[(ER_NEGPOS == "P" | PR_NEGPOS == "P")&HER2_NEGPOS == "N",mol := "HR+HER2-"]
meta[(ER_NEGPOS == "N" & PR_NEGPOS == "N")&HER2_NEGPOS == "P",mol := "HR-HER2+"]
meta[(ER_NEGPOS == "N" & PR_NEGPOS == "N")&HER2_NEGPOS == "N",mol := "TripleNeg"]

setnames(meta, "LFDN","PID")
meta[,PID := as.character(PID)]

#So it doesn't mess with model later (in case of log transforms)
meta[OS == 0,OS := 1]

#Limit to only those we have samples for
sub = meta[PID %in% unique(d$PID),]

#How many deaths
nrow(sub[censoring == 1,])
nrow(sub[(`Reconsent Ja/Nein` == "verstorben"),])
...
#Check Grade survival
par(mfrow=c(3,2))
for (i in sort(unique(sub$G))){
  cur_dat = subset(sub, sub$G == i)
  if (nrow(cur_dat) > 1){
  SurvObj_cur <- Surv(cur_dat$OS, cur_dat$censoring)
  km.cur.group <- survfit(SurvObj_cur ~ 1)
  plot(km.cur.group,mark.time = T,col = "red",xlim=range(1:210))
  par(new=TRUE)
  plot(km.as.one,mark.time = T,col = "black",xlim=range(1:210))
  legend("bottomleft",1,i,"red", cex=0.5)}
}

#N
par(mfrow=c(3,2))
for (i in sort(unique(sub$pN))){
  cur_dat = subset(sub, sub$pN == i)
  if (nrow(cur_dat) > 1){
  SurvObj_cur <- Surv(cur_dat$OS, cur_dat$censoring)
  km.cur.group <- survfit(SurvObj_cur ~ 1)
  plot(km.cur.group,mark.time = T,col = "red",xlim=range(1:210))
...
#mol
par(mfrow=c(3,2))
for (i in sort(unique(sub$mol))){
  cur_dat = subset(sub, sub$mol == i)
  if (nrow(cur_dat) > 1){
  SurvObj_cur <- Surv(cur_dat$OS, cur_dat$censoring)
  km.cur.group <- survfit(SurvObj_cur ~ 1)
  plot(km.cur.group,mark.time = T,col = "red",xlim=range(1:210))
...
sub[,nG := .N,by = "G"]
sub[,nN := .N,by = "pN"]
sub[,nmBC := .N,by = "censoring"]
sub[,nmol := .N,by = "mol"]

p = unique(sub[,c("G","nG")])
g1 = ggplot(p,aes(x = as.factor(1),y = nG, fill = G))+geom_bar(stat = "identity")+scale_fill_grey()+
  theme(aspect.ratio = 2)+
  ggtitle("Grade")

p = unique(sub[,c("pN","nN")])
g2 = ggplot(p,aes(x = as.factor(1),y = nN, fill = pN))+geom_bar(stat = "identity")+scale_fill_grey()+
  theme(aspect.ratio = 2)+
  ggtitle("pN")

        
p = unique(sub[,c("mol","nmol")])
g3 = ggplot(p,aes(x = as.factor(1),y = nmol, fill = mol))+geom_bar(stat = "identity")+scale_fill_grey()+
  theme(aspect.ratio = 2)+
  ggtitle("mol")
...
Average marker expression and fraction high expressing per patient
```{r}
#Average expressions per patient
avgs <- unique(d[,c("PID","marker","avgImage","Image")])
avgs[,avgPID := mean(avgImage,na.rm = T), by = c("PID","marker")]
avgs[,maxPID := max(avgImage,na.rm = T), by = c("PID","marker")]#Use highest value for each marker because sometimes certain LNs stain badly or contain no real tumor cells
avgs <- unique(avgs[,-c("Image","avgImage")])
avgs <- dcast.data.table(avgs,"PID ~ marker",value.var = "maxPID")

# #Fix NAs as in the middle of the negative peak when negative but blurred
# avgs[PID == "3823",HER2 := 0.1]
# avgs[PID == "3198",GATA3 := 0.1]

cor(na.omit(avgs[,-"PID"]))

#Fraction highly expressing per patient
frac <- unique(d[,c("PID","marker","frac","HiLo","Image")])
frac <- frac[HiLo == "high",]
frac[,avgfracPID := mean(frac,na.rm = T), by = c("PID","marker")]
frac[,maxfracPID := max(frac,na.rm = T), by = c("PID","marker")]
frac <- unique(frac[,-c("Image","frac","HiLo")])
frac <- dcast.data.table(frac,"PID ~ marker",value.var = "maxfracPID")
frac[is.na(frac)] = 0

cor(na.omit(frac[,-"PID"]))

```

Survival avg tumor cell expressions of p53 and GATA3
```{r}
#Try regular coxph without accounting for non-proportional hazards over time of p53
comb <- merge(avgs,meta,all.x = T,by = "PID")

res.cox1 <- coxph(Surv(OS, censoring) ~ p53, data =  comb)
summary(res.cox1)

res.cox2 <- coxph(Surv(OS, censoring) ~ GATA3, data =  comb)
summary(res.cox2)

res.cox3 <- coxph(Surv(OS, censoring) ~ GATA3 + p53, data =  comb)
summary(res.cox3)

#Extract model output
td_coef = as.data.table(tidy(res.cox3,exponentiate = T))
td_coef = td_coef[order(estimate),]
td_coef$term = factor(td_coef$term,levels = td_coef$term)
...
#Test assumptions
test.ph <- cox.zph(res.cox3) #p53 does not have proportional hazard -> residuals have significant relationship wit time
test.ph
ggcoxzph(test.ph[2])

#Make time strata for p53 -> treat as time dependent coefficient (https://cran.r-project.org/web/packages/survival/vignettes/timedep.pdf, page 16 -> step functions)
comb2 <- survSplit(Surv(OS, censoring) ~ ., data= comb, cut=c(30),
episode= "tgroup", id="id")
stratfit <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup), data=comb2)
summary(stratfit)

#Extract model output
td_coef = as.data.table(tidy(stratfit,exponentiate = T))
td_coef = td_coef[order(estimate),]
td_coef$term = factor(td_coef$term,levels = td_coef$term)
...
#Alternatively time transform functionality https://cran.r-project.org/web/packages/survival/vignettes/timedep.pdf page 20
vfit3 <- coxph(Surv(OS, censoring) ~ GATA3 + p53 + tt(p53),
data=comb,tt = function(x, t, ...) x * log(t+10))
summary(vfit3)

#Extract model output
td_coef = as.data.table(tidy(vfit3,exponentiate = T))
td_coef = td_coef[order(estimate),]
td_coef$term = factor(td_coef$term,levels = td_coef$term)
...
#Show survival curves for high/low p53
ggplot(comb,aes(x = p53))+geom_density()
comb$p53high = "no"
comb[p53 > 0.3,p53high := "yes"]

fit <- survfit(Surv(OS, censoring) ~ factor(p53high), data =  comb)
ggsurvplot(fit, data = comb,conf.int = FALSE,pval = TRUE,risk.table = TRUE)


#Show survival curves for high/low GATA3
ggplot(comb,aes(x = GATA3))+geom_density()
comb$GATA3high = "no"
comb[GATA3 > 0.4,GATA3high := "yes"]

fit <- survfit(Surv(OS, censoring) ~ factor(GATA3high), data =  comb)
ggsurvplot(fit, data = comb,conf.int = FALSE,pval = TRUE,risk.table = TRUE)

#Show survival curves for high GATA3 and low p53
comb$progn <- NULL
comb[GATA3high == "yes" & p53high == "no",progn := "good"]
comb[GATA3high == "no" & p53high == "yes",progn := "bad"]

fit <- survfit(Surv(OS, censoring) ~ progn, data =  comb)
ggsurvplot(fit, data = comb,conf.int = FALSE,pval = TRUE,risk.table = TRUE)


```

Combine with clinical classifications
```{r}
#Set ref levels
comb[, G := factor(G,levels = c("G3","G2","G1"))]
comb[,mol := factor(mol,levels = c("HR+HER2-","TripleNeg","HR+HER2+","HR-HER2+"))]
...
#Without pN
stratfit <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup) + G + mol, data=comb2)
summary(stratfit)
...
#Does adding p53 and GATA3 improve the model? Subset to only those with p53 and GATA3 so models have same size
comb2 = as.data.table(comb2)
comb2 <- comb2[!is.na(p53) & !is.na(GATA3),]
fit1 <- coxph(Surv(tstart, OS, censoring) ~  G + mol, data=comb2)
fit2 <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup) + G + mol, data=comb2)

#Yes, it does improve the model!
anova(fit1,fit2)


#With pN
stratfit <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup) + G + mol + pN, data=comb2)
summary(stratfit)
...
#Does adding p53 and GATA3 improve the model? Subset to only those with p53 and GATA3 so models have same size
fit1 <- coxph(Surv(tstart, OS, censoring) ~  G + mol +pN, data=comb2)
fit2 <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup) + G + mol + pN, data=comb2)

#Yes, it does improve the model! And only if both are added (not just p53)
anova(fit1,fit2)


#Show boxplots of p53 expressions across clinical groups
long <- melt.data.table(comb[,c("PID","mol","G","pN","ER","PR","HER2.x","Ki67","GATA3","p53")],id.vars = c("PID","mol","G","pN"),variable.name = "marker")
ggplot(long,aes(x = mol,y = value, fill = marker))+geom_boxplot()
ggplot(long,aes(x = G,y = value, fill = marker))+geom_boxplot()
ggplot(long,aes(x = pN,y = value, fill = marker))+geom_boxplot()

```

Combine with clinical markers in LN
```{r}
#markers only
stratfit <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup) + ER + PR + HER2.x + Ki67, data=comb2)
summary(stratfit)
...
#Does adding p53 and GATA3 improve the model?
fit1 <- coxph(Surv(tstart, OS, censoring) ~  ER + PR + HER2.x + Ki67, data=comb2)
fit2 <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup) + ER + PR + HER2.x + Ki67, data=comb2)

#Nope
anova(fit1,fit2)


#Accounting for non-proportional hazards of Ki67 -> p53 and GATA3 add nothing anymore
test.ph <- cox.zph(stratfit) #Ki67 also does not have proportional hazard
test.ph
ggcoxzph(test.ph[5])

#markers only
stratfit <- coxph(Surv(tgroup, OS, censoring) ~ GATA3 + Ki67:strata(tgroup) + p53:strata(tgroup) + ER + PR + HER2.x, data=comb2)
summary(stratfit)
...
#Does adding p53 and GATA3 improve the model?
fit1 <- coxph(Surv(tstart, OS, censoring) ~  ER + PR + HER2.x + Ki67:strata(tgroup), data=comb2)
fit2 <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup) + ER + PR + HER2.x + Ki67:strata(tgroup), data=comb2)

#Nope
anova(fit1,fit2)

#Including clinical classifications
stratfit <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup) + ER + PR + HER2.x + Ki67+ G + mol + pN, data=comb2)
summary(stratfit)
...
#Does adding p53 and GATA3 improve the model?
fit1 <- coxph(Surv(tstart, OS, censoring) ~  ER + PR + HER2.x + Ki67+ G + mol + pN, data=comb2)
fit2 <- coxph(Surv(tstart, OS, censoring) ~ GATA3 + p53:strata(tgroup) + ER + PR + HER2.x + Ki67+ G + mol + pN, data=comb2)
...
Check out PCA across markers
```{r}
library(factoextra)
p <- comb[,c("ER","PR","HER2.x","Ki67","GATA3","p53")]

res.pca <- prcomp(na.omit(p), scale = TRUE)
fviz_eig(res.pca)
...
#Survival
combnona <- comb[!is.na(ER) & !is.na(PR) & !is.na(HER2.x) & !is.na(GATA3) & !is.na(Ki67) & !is.na(p53)]
combnona <- cbind(combnona[,-c("ER","PR","HER2.x","Ki67","GATA3","p53")],res.pca$x)


#Including clinical classifications
fit <- coxph(Surv(OS, censoring) ~ PC1 + PC2 + PC3, data=combnona)
summary(fit)

#Extract model output
td_coef = as.data.table(tidy(fit,exponentiate = T))
td_coef = td_coef[order(estimate),]
td_coef$term = factor(td_coef$term,levels = td_coef$term)
...
#Try without p53 and GATA3
p <- comb[,c("ER","PR","HER2.x","Ki67")]
...
#Survival
combnona <- comb[!is.na(ER) & !is.na(PR) & !is.na(HER2.x) & !is.na(Ki67)]
combnona <- cbind(combnona[,-c("ER","PR","HER2.x","Ki67","GATA3","p53")],res.pca$x)
